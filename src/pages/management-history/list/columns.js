...
export const columnsDeposit = (
  ...
  },
  {
    ...
    cell: row => (
      <Link to={`/customer/detail/${row?.appUserId}`} className='user-name text-truncate mb-0'>
...

  {
    ...
    cell: row => (
      <Badge
        ...
        color={
          statusObj[
            row?.paymentStatus === 'Completed' ? 'active' : row?.paymentStatus === 'Canceled' ? 'notActive' : ''
          ]
        }
      >
    ...
    cell: row => (
      <>
          ...
          <div>
            <Check
              ...
              onClick={() => {
                ...
                handleIsOpenModal(true, row?.[handleChangeTypeHistory(typeHistory)])
              }}
            />
            <X
              color={'#E74160'}
        ...
        <Eye
          ...
          onClick={() => history.push(`/history/detail/${typeHistory}/${row?.[handleChangeTypeHistory(typeHistory)]}`)}
        />
      </>
    )
  }
      ...
      <div style={{ overflow: 'hidden' }}>
        {row?.cryptoWalletAddress && (
          <>
            ...
            >
              Địa chỉ ví : {row?.cryptoWalletAddress}
            </span>
            <br />
          </>
        )}
...
export const columnsBetRecords = (
...
) => [
  {
    ...
    minWidth: '70px',
    maxWidth: '160px',
    cell: row => <span>{row?.betRecordSection}</span>
  },
  {
    name: 'TÀI KHOẢN',
    minWidth: '250px',
  ...
  },
  {
    ...
    selector: 'paymentAmount',
    center: true,
    cell: row => (row?.betRecordType === 'BetUp' ? 'Tăng' : row?.betRecordType === 'BetDown' ? 'Giảm' : '')
  },
  {
    name: 'KẾT QUẢ',
    minWidth: '150px',
    ...
    cell: row => (
      <Badge
        className='text-capitalize'
        color={
          statusObj[row?.betRecordResult === 'win' ? 'active' : row?.betRecordResult === 'lose' ? 'notActive' : '']
        }
      >
        {row?.betRecordResult === 'win' ? 'Thắng' : row?.betRecordResult === 'lose' ? 'Thua' : ''}
      </Badge>
    )
  },
  {
